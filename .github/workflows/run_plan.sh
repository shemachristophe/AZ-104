...
function get_terraform_plan_return_message(){
  ...
  terraform plan -detailed-exitcode -out plan.tfplan 1>stdout 2>err.txt | grep 'Error: Terraform exited with code 2.' &> /dev/null && echo 'Terraform exit Code 2: Changes Detected!'
  #echo 'err' &2
  exitCode=$?
  cat stdout
  #echo'_------------------_'
